...
def test_foreign_scope_uses_title_tokens_and_mixed_tokens_are_comprehensive():
    ...
    assert guess_scope("Digital medical device requirements", title="Digital medical device guidance", publisher="MDCG (EU)") == "디지털 의료기기"
    assert guess_scope("Digital Executive Agency (HaDEA) and medical devices.", title="Study supporting availability of medical devices", publisher="MDCG (EU)") == "종합"
    ...
    assert guess_scope(body, title="Digital transformation guidance", publisher="MDCG (EU)") == "종합"
