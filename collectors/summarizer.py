...
def guess_scope(text: str, *, title: str | None = None, publisher: str | None = None) -> str:
    ...
    if publisher and re.search(r"\bMDCG\b", publisher, re.IGNORECASE):
        ...
        if re.search(r"digital(?:\W+\w+){0,2}\W+medical\s+devices?", mdcg_value, re.IGNORECASE):
            mdcg_scopes.add("디지털 의료기기")
        if re.search(r"\bmedical\s+devices?\b", mdcg_value, re.IGNORECASE):
            mdcg_scopes.add("일반 의료기기 범주")
        if mdcg_scopes == {"일반 의료기기 범주"}:
            return "종합"
        mdcg_scopes.discard("일반 의료기기 범주")
